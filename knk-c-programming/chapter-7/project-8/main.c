...
int main()
{
	...
	printf("Enter a 12-hour time(hours:minutes (AM or PM)): ");
	...
	if(am_pm == 'A') {
		...
		converted_time = (input_hour + 12) * 60 + input_minute;
	}
	...
	if(converted_time < DEPARTURE_TIME_1) {
		...
		if(compare_with_past < 0)
			compare_with_past = -compare_with_past; if(compare_with_comming < 0)
